...
namespace MFramework.Domain.Services.Repository.EF
{
    ...
    /// </summary>
    public class EFRepository<TEntity> : RepositoryBase<TEntity> where TEntity : Entity<TEntity>, new()
    {
        private IEFSession _privateSession;
        ...
        {
             if (MFrameworkServiceLocator.Current == null) 
                return;

             var sessions = MFrameworkServiceLocator.Current.GetAllInstances<IEFSession>();
        ...
        public override IEnumerable<TEntity> Select(Expression<Func<TEntity, bool>> whereClause = null, IOrderByClause<TEntity>[] orderBy = null, int skip = 0, int top = 0,
            string[] include = null)
        {
            throw new NotImplementedException();
        }
